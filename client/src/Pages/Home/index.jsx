...
import CatSliderLoading from "../../components/CatSliderLoading";
...
const Home = () => {
 ...
 useEffect(() => {
  if (context?.catData?.length > 0) {
    setIsPopularLoading(true);   // ✅ START LOADING FIRST

    fetchDataFromApi(`/api/product/getAllProductsByCatId/${context.catData[0]._id}`)
      .then((res) => {

        if (res?.success === true && res?.products?.length > 0) {
          setPopularProductsData(res.products);
        } else {
          setPopularProductsData([]); // ✅ clear on empty
        }

        setIsPopularLoading(false);  // ✅ STOP LOADING
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    // Assume catData loads via context; poll or listen for it
    const checkCatData = () => {
      if (context?.catData?.length > 0) {
        context?.setIsCatLoading(false);
      }
    };
    checkCatData(); // Initial check

    // If context updates trigger re-renders, use a listener or just check in render
  }, [context?.catData]);
  ...
  return (
    <>
      <div className=" min-h-screen">
        {/* ===== Banner Section ===== */}
        <div className="relative min-h-[150px] md:min-h-[100px] lg:min-h-[450px]">
          {" "}
          {/* Reserves slider space */}
          {isBannerLoading ? (
            <BannerLoading /> // Absolute inside relative: perfect overlay, no flow break
          ) : (
            homeSlidesData?.length !== 0 && <HomeSlider data={homeSlidesData} />
          )}
        </div>

        {/* Cat Section: Now with reserved space & skeleton */}
        <div className="min-h-[50px] md:min-h-[250px] lg:min-h-[150px]">
          {" "}
          {/* Reserves cat space; adjust height */}
          {context?.catData?.length !== 0 && !context?.isCatLoading ? (
            <HomeCatSlider data={context?.catData} />
          ) : (
            <CatSliderLoading /> // Renders while loading/empty: no shift!
          )}
          {/* If you had a separate banner loading for this area (e.g., ads below cats), add it here conditionally:
  {isSomeOtherLoading && <OldCatBannerLoading />} */}
        </div>
          ...
          <div className="container-fluid">
            <div className="flex items-center justify-between flex-col lg:flex-row gap-4">
              <div className="leftSec w-full lg:w-[40%]">
                <h3 className="text-[16px] px-4 sm:px-1 sm:text-[16px] md:text-[18px] lg:text-[22px] font-bold">
                  Popular Products
                </h3>
                <p className="text-[14px] px-4 sm:px-1 sm:text-[16px] md:text-[16px] lg:text-[18px] !mt-0 !mb-0 text-gray-600">
                  Do not miss the current offers until the end of this Month
                </p>
              </div>
  ...
  {
    isPopularLoading == true ? <ProductLoading /> :
    popularProductsData?.length !== 0 ? (
      <ProductSlider data={popularProductsData} items={6} />
    ) : <>
   <div className="text-center h-full  bg-red-100 flex items-center text-red-600 rounded-md px-4 py-2">
      <img src="/no-product-found.png" className="flex items-center" alt="" />
    </div>
    </>
  }
</div>
    ...
    <div className="part2 scrollableBox w-full lg:w-[30%] flex flex-col gap-5">
      <BannerboxV2
 ...
 <div className="container-fluid">  {bannerList2Data?.length !== 0 && (
              <AdsBannerSliderV2 data={bannerList2Data} items={4} />
            )}</div>
