import catBannerModel from "../models/catBanner.model.js";
...
export async function getBanners(request, response) {
  try {
    const banners = await catBannerModel.find();
...
export async function deleteBanner(request, response) {
  try {
    ...
    const banner = await catBannerModel.findById(request.params.id);
    if (!banner) {
    ...
    const deletedBanner = await catBannerModel.findByIdAndDelete(
      request.params.id
    );
...
export async function updatedBanner(request, response) {
  const banner = await catBannerModel.findByIdAndUpdate(
    request.params.id,
    {
      // bannerTitle: request.body.bannerTitle,
...
export async function deleteMultipleProduct(request, response) {
  ...
  for (let i = 0; i < ids?.length; i++) {
    const banner = await catBannerModel.findById(ids[i]);
  ...
  try {
    await catBannerModel.deleteMany({
      _id: {
        $in: ids,
      },
    });
