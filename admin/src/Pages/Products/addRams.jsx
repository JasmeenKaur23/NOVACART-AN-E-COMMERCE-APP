...
const AddRams = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editId === "") {
      postData(`/api/product/productRAMS/create`, { name: name }).then(
        (res) => {
          if (res?.error === false) {
    ...
    if (editId !== "") {
      editData(`/api/product/productRAMS/${editId}`, { name: name }).then(
        (res) => {
            ...
            <table class="w-full text-sm mb-4 text-left rtl:text-right text-gray-500 dark:text-gray-400">
              <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  <th
                    scope="col"
                    className="px-8 whitespace-nowrap py-3"
                    width="60%"
                  >
                    PRODUCT RAM
                  </th>
                  ...
                  return (
                    ...
                    >
                      <td className="px-8 py-2">
                        <span className="text-[#4f4e4e] font-[600]">
                          {item?.name}
                        </span>
                      </td>
